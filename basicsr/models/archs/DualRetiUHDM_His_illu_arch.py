...
from basicsr.models.archs.UHDM_His_illu import UHDM_His_illu
...
class Illumination_Estimator(nn.Module):
    ...
    def forward(self, img):
        ...
        cond_R = self.conv4(R_prime)
        cond_L = self.conv5(L_prime) 

        return R_prime, L_prime, L_bar, R_bar, cond_R, cond_L
...
class DualRetiUHDM_His_illu_Single_Stage(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, level=2, num_blocks=[1, 1, 1]):
        super(DualRetiUHDM_His_illu_Single_Stage, self).__init__()
        self.estimator = Illumination_Estimator(n_feat)
        #self.denoiser = Denoiser(in_dim=in_channels,out_dim=out_channels,dim=n_feat,level=level,num_blocks=num_blocks)
        self.denoiser_uhdm_R = UHDM_His_illu()
        self.denoiser_uhdm_L = UHDM_His_illu()
    def forward(self, img):
        #illu_fea, illu_map = self.estimator(img)
        #input_img = img * illu_map + img           #[B, 3, H, W]
        #output_img = self.denoiser(input_img,illu_fea)

        R_prime, L_prime, L_bar, R_bar, R_cond, L_cond = self.estimator(img)
        
        out_R1, out_R2, out_R3 = self.denoiser_uhdm_R(R_prime, R_cond)
        out_L1, out_L2, out_L3 = self.denoiser_uhdm_L(L_prime, L_cond) 

        return out_R1, out_R2, out_R3, out_L1, out_L2, out_L3


class DualRetiUHDM_His_illu(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        super(DualRetiUHDM_His_illu, self).__init__()
        self.stage = stage

        modules_body = [DualRetiUHDM_His_illu_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=2, num_blocks=num_blocks)
