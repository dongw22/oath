...
class IGHB(nn.Module):
    def __init__(self, dim, num_heads=1, ffn_expansion_factor=4, bias=False, LayerNorm_type='WithBias'):
        super(IGHB, self).__init__()

        self.attn_g = Attention_histogram_illu(dim, num_heads, bias, True)
        self.norm_g = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)
    ...
    def forward(self, x, illu_cond):
        ...
        x = x + self.attn_g(self.norm_g(x), illu_cond)

        x_normed = self.norm_ff1(x)
...
class Attention_histogram_illu(nn.Module):
    def __init__(self, dim, num_heads, bias, ifBox=True):
        super(Attention_histogram_illu, self).__init__()
        self.factor = num_heads
        self.ifBox = ifBox
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
