...
from basicsr.models.losses.onerestore_loss import msssim
...
class multi_VGGPerceptualLoss(torch.nn.Module):
    ...
    def forward(self, out1, out2, out3, gt1, feature_layers=[2]):
        ...
        #msssim_loss1 = 1-msssim(out1, gt1, normalize=True)
        #msssim_loss2 = 1-msssim(out2, gt2, normalize=True)
        #msssim_loss3 = 1-msssim(out3, gt3, normalize=True)

        loss1 = 1* l1_loss1 + per_loss1 #+ 0.4 * msssim_loss1
        loss2 = 1 * l1_loss2 + per_loss2 #+ 0.4 * msssim_loss2
        loss3 = 1* l1_loss3 + per_loss3 #+ 0.4 * msssim_loss3

        loss_dict = OrderedDict()

        if loss_dict is not None:
            loss_dict['cb_loss'] = l1_loss1 
            # loss_dict['COS_loss'] = COS_loss1 + COS_loss2 + COS_loss3
            loss_dict['per_loss'] = per_loss1 
            #oss_dict['msssim'] = msssim_loss1 + msssim_loss2 + msssim_loss3
